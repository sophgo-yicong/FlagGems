...
def _launch_bool_tensor(a, b, out):
    ...
    # logical AND torch.bitwise_and defines for bool. Fully self-contained (no
    ...
    if n % block_size == 0:
        ...
        _bitwise_and_tensor_contig_kernel[grid](ai, bi, oi, n, BLOCK_SIZE=block_size)
    return out
...
def _launch_bool_scalar(a, value, out):
    ...
    if n % block_size == 0:
        ...
        _bitwise_and_scalar_contig_kernel[grid](ai, v, oi, n, BLOCK_SIZE=block_size)
    return out
...
# kernel here. We never fall back to torch.* / .bitwise_and_ for a TPU tensor,
