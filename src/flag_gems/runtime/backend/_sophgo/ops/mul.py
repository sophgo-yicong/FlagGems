...
from flag_gems.ops.mul import mul as _generic_mul, mul_ as _generic_mul_
...
@triton.jit
def mul_tt_kernel_fast(x_ptr, y_ptr, out_ptr, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
...
@triton.jit
def mul_tt_kernel_masked(x_ptr, y_ptr, out_ptr, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
        offs = (pid + t * tl.num_programs(0)) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        mask = offs < n
...
def _run_tt(a, b, out):
    ...
    k = mul_tt_kernel_fast if n % BLOCK_SIZE == 0 else mul_tt_kernel_masked
    k[(grid,)](a, b, out, n, BLOCK_SIZE=BLOCK_SIZE, TPB=tpb)
    return out


def mul(A, B):
    logger.debug("GEMS MUL (sophgo_tpu)")
    if isinstance(A, torch.Tensor) and isinstance(B, torch.Tensor) and _can_fast(A, B):
        return _run_tt(A, B, torch.empty_like(A))
    return _generic_mul(A, B)


def mul_(A, B):
    logger.debug("GEMS MUL_ (sophgo_tpu)")
    if isinstance(B, torch.Tensor) and _can_fast(A, B):
        return _run_tt(A, B, A)
    return _generic_mul_(A, B)
