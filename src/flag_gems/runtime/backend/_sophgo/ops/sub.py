...
from flag_gems.ops.sub import sub as _generic_sub, sub_ as _generic_sub_
...
@triton.jit
def sub_tt_kernel_fast(x_ptr, y_ptr, out_ptr, alpha, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
...
@triton.jit
def sub_tt_kernel_masked(x_ptr, y_ptr, out_ptr, alpha, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
        offs = (pid + t * tl.num_programs(0)) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        mask = offs < n
