...
# ---- scalar weight ---------------------------------------------------------
@triton.jit
def _lerp_scalar_head_fast(in_ptr, end_ptr, out_ptr, w, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
...
@triton.jit
def _lerp_scalar_head_masked(in_ptr, end_ptr, out_ptr, w, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
        offs = (pid + t * tl.num_programs(0)) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        mask = offs < n
...
@triton.jit
def _lerp_scalar_tail_fast(in_ptr, end_ptr, out_ptr, w, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
        offs = (pid + t * tl.num_programs(0)) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        a = tl.load(in_ptr + offs).to(tl.float32)
...
@triton.jit
def _lerp_scalar_tail_masked(in_ptr, end_ptr, out_ptr, w, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
        offs = (pid + t * tl.num_programs(0)) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        mask = offs < n
...
# ---- tensor weight ---------------------------------------------------------
@triton.jit
def _lerp_tensor_fast(in_ptr, end_ptr, w_ptr, out_ptr, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
        offs = (pid + t * tl.num_programs(0)) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        a = tl.load(in_ptr + offs).to(tl.float32)
...
@triton.jit
def _lerp_tensor_masked(in_ptr, end_ptr, w_ptr, out_ptr, n, BLOCK_SIZE: tl.constexpr, TPB: tl.constexpr):
    pid = tl.program_id(0)
    for t in range(TPB):
        offs = (pid + t * tl.num_programs(0)) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        mask = offs < n
