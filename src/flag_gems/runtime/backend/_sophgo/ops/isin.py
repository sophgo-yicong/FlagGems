...
def isin_by_search(
    ...
    if unique_in0:
        ...
        # )
        in0_ravel, unique_order = torch.unique(
            in0.cpu(), sorted=True, return_inverse=True, return_counts=False
        )
        in0_ravel = in0_ravel.to(in0.device)
    else:
        in0_ravel = in0.contiguous().ravel()
    if unique_in1:
        ...
        # )
        in1_ravel_cpu = torch.unique(
            in1.cpu(), sorted=True, return_inverse=False, return_counts=False
        )
        in1_ravel = in1_ravel_cpu.to(in1.device)
    ...
    if M <= 1048576:  # 2 ** 20 = 1024 * 1024
        ...
        _, BLOCK_M, num_warps = launch_arg(None, 2048, M, 32)
    else:
        _, BLOCK_M, num_warps = launch_arg(None, 2048, M, 16)
